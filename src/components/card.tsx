    ...
    <Box>
      <TouchableOpacity onPress={() => setInfoPokeBoolean(true)}>
        ...
        >
          <ImageBackground
            borderRadius={20}
            source={require("../assets/pokeballFundo.jpg")}
            w={"100%"}
            h={"100%"}
            alignItems="center"
          ...
          >
            <Image
              source={image ? { uri: image } : require("../assets/erro404.png")}
              alt={uniquePoke.name}
              resizeMode="center"
            />

            <HStack bottom={-20} gap={10}>
              <Text right={0} color="$white" fontWeight={"$bold"}>
                {uniquePoke.name}
              </Text>
              <Text left={0} color="$white" fontWeight={"$bold"}>
                {uniquePoke.id}#
              </Text>
            </HStack>
          </ImageBackground>
      ...
      <InfoPoke
        isModal={infoPokeBoolean}
        setIsModal={setInfoPokeBoolean}
        poke={uniquePoke}
      />
    </Box>
  );
